...
public:
  ...
  */
  QboDuinoDriver(std::string port1 = "/dev/ttyUSB0", int baud1 = 115200, std::string port2 = "/dev/ttyUSB1", int baud2 = 115200, float timeout1 = 0.01, float timeout2 = 0.01);
  // Public functions
  ...
  int getIMU(int16_t &gyroX, int16_t &gyroY, int16_t &gyroZ, int16_t &accelerometerX, int16_t &accelerometerY, int16_t &accelerometerZ);
  int calibrateIMU(uint8_t &result);
  int resetStall();
  int getMotorsState(uint8_t &state);
  int getInfraRed(uint8_t &ir0, uint8_t &ir1, uint8_t &ir2);
